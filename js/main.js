...
//         box += `
// //         <div class="col-md-3">
// //                     <div id="" class=" rounded-2 cursor-pointer text-white ">
// //                         <img class="w-100" src="imgss/logo.png" alt="" srcset="">
...
//     let Letter = await fetch (`https://www.themealdb.com/api/json/v1/1/search.php?f=${term}`);
...
async function getClickOnCategories(get) {
    let response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${get}`)
    response = await response.json()
    // console.log(response.categories);
    // displayCategories(response.categories);
// console.log(response.meals)
...
function displayIngredient(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        box += `
        <div class="col-md-3">
                <div onclick="getMainGredient('${arr[i].strIngredient}')" class="rounded-2 text-center cursor-pointer">
...
async function getMealArea(located) {
    let response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${located}`)
    response = await response.json()
    console.log(response.meals);
    // displayArea(response.meals)
    displayMeals(response.meals);
...
function Validation() {
    ...
    document.getElementById('repass').classList.toggle('d-none', isRePasswordValid);
